...
void Input::process_viewport_input() {
    ...
    float& deltaTime = unstable_engine->delta_time_seconds;
    bool& firstMouse = unstable_engine->firstMouse;
    ...
    if (transforming_selected_object) {
        if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
            ...
            if (rendering->transform3d->type != TRANSLATION) {
                glfwSetInputMode(unstable_engine->window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
            }
            transforming_selected_object = false;
        }
    }
        ...
        if (selected_object_transform3d != nullptr) {
            ...
            if (ImGui::IsMouseClicked(ImGuiMouseButton_Left, false)) {
                if (rendering->transform3d->type != TRANSLATION) {
                    glfwSetInputMode(unstable_engine->window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
                }
...
void Input::mouse_rotate_camera() {
    Camera* camera_viewport = rendering->camera_viewport;
    bool& firstMouse = unstable_engine->firstMouse;
    float& lastX = unstable_engine->lastX;
    float& lastY = unstable_engine->lastY;
