...
UnstableEngine* UnstableEngine::instance = nullptr;
std::mutex UnstableEngine::neon_engine_mutex;

UnstableEngine::UnstableEngine() {
    window = nullptr;
...
void UnstableEngine::initialize_all_components() {
...
UnstableEngine* UnstableEngine::get_instance()
{
    std::lock_guard<std::mutex> lock(neon_engine_mutex);
    if (instance == nullptr) {
        instance = new UnstableEngine();
    }
    return instance;
}
